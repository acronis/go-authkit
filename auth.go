...
func NewJWTParser(cfg *Config, opts ...JWTParserOption) (JWTParser, error) {
	...
	if options.httpUserAgent == "" {
		options.httpUserAgent = libinfo.UserAgent()
	} else {
		options.httpUserAgent = fmt.Sprintf("%s %s", options.httpUserAgent, libinfo.UserAgent())
	}
	...
	httpClient := idputil.MakeDefaultHTTPClient(
		...
		options.requestIDProvider, options.httpUserAgent,
	)
...
// WithJWTParserHTTPUserAgent sets the User-Agent header for HTTP requests to JWKS endpoint.
func WithJWTParserHTTPUserAgent(userAgent string) JWTParserOption {
	return func(options *jwtParserOptions) {
